# Meeting minutes - ICEG Review Committee - 2026-05-21 (Brussels, BOSA)

Attendees:

- Vlaanderen: eveline.vlassenroot@vlaanderen.be; jitse.decock@vlaanderen.be
- BOSA: liesbet.dhondt@bosa.fgov.be; marc.bruyland@bosa.fgov.be; bart.hanssens@bosa.fgov.be;
- Brussels Gewest: ;
- Fédération Wallonie-Bruxelles: eauquiere@paradigm.brussels;
- Région wallonne: dominique.buffet@spw.wallonie.be;
- Deutschsprachige Gemeinschaft: /
- NGI: rink.kruk@ngi.be

Guests:
yaron.dassonneville@vlaanderen.be; valy.vandenbroeck@vlaanderen.be

Excused:
- Vlaanderen: ziggy.vanlishout@vlaanderen.be; pauline.dewolf@vlaanderen.be
- BOSA: / 
- Brussels Gewest : bfricheteau@paradigm.brussels; ahof@gob.brussels; thoogewys@paradigm.be;
- Fédération Wallonie-Bruxelles: alexandre.delalieu@cfwb.be; 
- Région wallonne: xavier.demarets@spw.wallonie.be
- Deutschsprachige Gemeinschaft: dominique.buecken@dgov.be
- NGI: /
- Guests: Jonas De Meulenaere (fedvoc werkgroep)

## AGENDA
- Tour de Table
- Update Data standardisation exercises:
  1) ICEG Person
  2) ICEG Hydrants
  3) Assessment Best
- Update translation exercise
- Topics for ICEG Management Meeting 5/06
- Varia
      


## MEETING MINUTES
- Update Data standardisation exercises:
  1) ICEG Person
  2) ICEG Hydrants
  3) Assessment Best
- Update translation exercise
- Topics for ICEG Management Meeting 5/06
- Varia:
    - Question from SEMIC team with regards to CPSV-Ap, see https://github.com/SEMICeu/CPSV-AP/issues/139
      "How are the different MS handling this issue, how are they dealing with local services?"
      Mention procedures-steps (OSLO Steps)?
      ACTION: Give feedback directly via Github.
    - Event on Image Interoperability in Brussels: https://fedosc.miraheze.org/wiki/Events/InterFSIIIIF
    - Question Frank Leyman: Which new exercises are starting?
    - Paradigm: Can we make examples to illustrate the relevance of the topics of this working group?
      We need to extend the role of this forum to share experiences on how to implement interoperability.
      Proposal: "Interop Day"
      Possible topics:
      - Rest Style Guide: tech working group (peter.vandenbosch@smals.be) together with Rest guidelines VDIP in Flanders  (Laurens Debackere)?
      - Open Science Cloud initiative: https://fedosc.miraheze.org/wiki/Events/InterFSIIIIF
      - Data spaces and interoperability - together with BDSA?
     
Next meeting: 
3/09/2026, host: Paradigm

